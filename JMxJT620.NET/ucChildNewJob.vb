...
Public Class ucChildNewJob
    ...
    '==
    '==
    '==  NEW REVISION Built in DEVEL-- 4219 VERSION
    '==  NEW REVISION Built in DEVEL-- 4219 VERSION
    '==  NEW REVISION Built in DEVEL-- 4219 VERSION
    '==  NEW REVISION Built in DEVEL-- 4219 VERSION
    '==    Updated- 4219.1215 - Started 08-Dec-2019= 
    '==      --  MAKE Form "frmNewJob32" INTO USERCONTROL.
    '--            SO THAT we can show it as a Child in a POS TAB..
    '==
    '= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
    '= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
    '==
    ...
    '==           Make Calender XML update file into Function..
    '==            Can call from new?update Job, or from Cancel Job..
    '==
    '= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
    '==
    ...
    Private msCalendarUpdateXmlFileName As String = ""
    ...
    ReadOnly Property ExchangeCalendarUpdateXmlFileName As String
        Get
            ExchangeCalendarUpdateXmlFileName = msCalendarUpdateXmlFileName
        End Get
...
Receipt_error:
    ...
    '==
    '==    -- Cancel Job (frmNewJob etc)..  Update Calendar to delete the Appointment..
    '==           Make code for Calender XML update file into Function..
    '==            Can call from new/update Job, or from Cancel Job..
    '== = = = = = =

    '-- RequestType can be "NewJob", "UpdateJob", or "CancelJob"..

    Private Function mbSendCalendarUpdate(ByVal intJob_id As Integer,
                                          ByVal strRequestType As String,
                                          ByVal sDatePromised As String) As Boolean

        Dim datePromisedOld, datePromisedNew As Date
        Dim bIsNewJob As Boolean = (Not mbAmending)
        Dim sBody, sCust, sLocation As String

        mbSendCalendarUpdate = False
        ...
        '==  New Tag..
        sXmlFileXml &= "<appt_RequestType>"
        sXmlFileXml &= strRequestType
        sXmlFileXml &= "</appt_RequestType> " & vbCrLf
        ...
        Try
            ...
            msCalendarUpdateXmlFileName = sExchangeFullPath
            mbSendCalendarUpdate = True
            MessageBox.Show("NB: A Calendar Update Info XML descr. File " & vbCrLf &
                   " has been queued OK to the Data Dir..", "New/Amend..", MessageBoxButtons.OK, MessageBoxIcon.Information)
        Catch ex As Exception
            MessageBox.Show("ERROR- Failed to save Exchange20 XML descr. file to Data Dir..",
    ...
    End Function  '-mbSendCalendarUpdate--
    ...
    Private Sub mbOriginal_frmNewJob3_Load()
        ...
        '==  NEW REVISION as USERCONTROL-  Built in DEVEL-- 4219 VERSION
        '==Call CenterForm(Me)
    ...
    Private Sub frmNewJob3_Load(ByVal eventSender As System.Object, _
        ...
        With My.Application.Info.Version
            LabVersion.Text = msJobMatixAppName & " V:" & CStr(.Major) & "." & .Minor & "- Build: " & _
                                                                             .Build & "." & .Revision & "="
        End With
        labOnSiteTime.Visible = False
        timeOnSite.Visible = False
        ...
        Try
            ...
            mClsAttachments1 = New clsAttachments(New Form, mCnnJobs, "JOB", openDlg1)
        Catch ex As Exception
        ...
        '==  NEW REVISION as USERCONTROL-  Built in DEVEL-- 4219 VERSION
        '==  NEW REVISION as USERCONTROL-  Built in DEVEL-- 4219 VERSION
        '-- NOW call SHOWN..  (we are a USERCONTROL now.)

        Call frmNewJob3_Shown(eventSender, eventArgs)
    ...
    Private Sub frmNewJob3_Shown(ByVal eventSender As System.Object, _
        ...
        LabHdr2.Text = msBusinessName & "  " & msBusinessState & " " & msBusinessPostCode
    ...
    Private Sub cmdFinish_Click(ByVal eventSender As System.Object, _
        ...
        If mbIsOnSiteJob And ((Not mbAmending) Or (mbAmending And mbDataChanged)) Then
            ...
            If mClsSystemInfo.exists("EXCHANGE_MAILBOX_USER") AndAlso
                                  (mClsSystemInfo.item("EXCHANGE_MAILBOX_USER") <> "") AndAlso
                                                                          IsDate(sDatePromised) Then

                '==Build-4284..  see function.
                Dim sRequestType As String = "NewJob"
                If mbAmending Then
                    sRequestType = "UpdateJob"
                End If
                If Not mbSendCalendarUpdate(mlJobId, sRequestType, sDatePromised) Then
                    MsgBox("Failed to lodge calendar Update..", MsgBoxStyle.Exclamation)
                End If
        ...
        '=4219.1215=
        mbDataChanged = False  '-can exit via parent.
...
cmdFinish_error:
    ...
    Private Sub cmdCancelJob_Click(ByVal eventSender As System.Object, _
        ...
        '== Target-Build-4284  (Started 04-Nov-2020)
        Dim bIsCalendarJob As Boolean = False

        If mbIsOnSiteJob Then
            If mClsSystemInfo.exists("EXCHANGE_MAILBOX_USER") AndAlso
                         (mClsSystemInfo.item("EXCHANGE_MAILBOX_USER") <> "") Then
                bIsCalendarJob = True
                sMsg &= vbCrLf & "  (Calendar appointment will be deleted..)"
            End If
        End If '--onSite-
        ...
        If mbAmending Then  '== mlCustomerId >= 0 Then '--started
            ...
            Else '--yes--
                ...
                Else '--ok-
                    System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.Default '--in case --
                    MsgBox("Job No: " & mlJobId & " has been Cancelled.. " & vbCrLf &
                                "( " & lngaffected & " Record(s) affected..)", MsgBoxStyle.Information)

                    '== Target-Build-4284  (Started 04-Nov-2020)
                    '--   Send XML file to Update Calandar for Delete.
                    If bIsCalendarJob Then
                        Dim sRequestType As String = "CancelJob"
                        Dim sDatePromised As String = Format(datePromised.Value, "dd-MMM-yyyy ") &
                                                              Format(TimeValue(timeOnSite.Value), "hh:mm tt")
                        If Not mbSendCalendarUpdate(mlJobId, sRequestType, sDatePromised) Then
                            MsgBox("Failed to lodge calendar Update..", MsgBoxStyle.Exclamation)
                        End If
                    End If  '-calendar job.-
