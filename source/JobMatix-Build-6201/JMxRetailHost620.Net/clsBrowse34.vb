...
Friend Class clsBrowse3
    ...
    '== = = = = =
    '==
    '== -- Updated 3501.1221  21-Dec-2018=  
    '==     --  Now In POS. Reformat Decimal (money) columns if requested...
    '==
    '= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
    ...
    '==  3501.1221=    
    Private mbReformatDecimalColumns As Boolean = False
    ...
    WriteOnly Property ReformatDecimalColumns As Boolean
        Set(value As Boolean)
            mbReformatDecimalColumns = value
        End Set
    End Property
    '= = = = = = = = = = = = = = = = = == = 
    ...
    Private Function mbLoadGrid(ByRef sqlRdr1 As OleDbDataReader) As Boolean
        ...
        If (lngRecCount > 0) Then
            ...
            '-- ====== FIX Customer names.--
            '=3501.1221--
            '= --ReformatDecimalColumns==
            If mbReformatDecimalColumns Then
                Dim decAmount As Decimal
                Try
                    For cx = 0 To (mDataTableCurrent.Columns.Count - 1)  '= rs.Fields.Count - 1
                        sName = mDataTableCurrent.Columns(cx).ColumnName  '=rs.Fields(cx).Name
                        If mDataTableCurrent.Columns(cx).DataType Is GetType(Decimal) Then
                            '= .Cells(k_PAYGRIDCOL_AMOUNT).Style.Alignment = DataGridViewContentAlignment.MiddleRight
                            For rx = 0 To (lngRecCount - 1)  '-tracking down the grid.--
                                datarow1 = mDataTableCurrent.Rows(rx)
                                decAmount = CDec(datarow1.Item(sName)) '--get rs original-
                                mDataGridView1.Rows(rx).Cells(cx).Style.Alignment = DataGridViewContentAlignment.MiddleRight
                                s1 = FormatCurrency(decAmount, 2)
                                s1 = Replace(s1, "$", "") '-drop dollar sign for grid..
                                mDataGridView1.Rows(rx).Cells(cx).Value = Replace(s1, ",", "")  '-drop for grid..
                            Next rx
                        End If  '-decimal-
                    Next cx
                Catch ex As Exception
                    MsgBox("Load-Grid- Error Reformatting Decimal Column." & vbCrLf & ex.Message & vbCrLf & _
                       vbCrLf & "Processing will contine..", MsgBoxStyle.Information)
                End Try
            End If  '-reformat decimal-
