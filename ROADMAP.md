...
- **SMS notifications** — ✅ done. 4 gateway adapters (SMS Boss, SMS Broadcast, SMSGlobal, DirectSMS) built and verified without ever contacting a real gateway (a fake HTTP handler confirmed request construction and response parsing for each, since no real credentials exist and a live send costs money and texts a real number). Settings live in the existing `systeminfo` key/value table (Staff tab, admin-gated); "Notify Customer" on the Jobs tab sends and logs onto `jobs.notifications`, matching the legacy append-to-job-record pattern.
- **Email notifications (SMTP)** — a natural sibling to SMS, same settings store and trigger pattern, independent of the Exchange/EWS calendar integration (which is being dropped, see Phase 0.3) — don't conflate the two, they're unrelated despite both being "email." Not yet built.
