...
4. **Two real gaps, verified by reading the actual routes (not assumed)**:
   - `franky-forge`'s `partnerProducts.js` is inbound-only today (a partner pushes their catalog *into* Franky for enrichment) — the wrong direction for this. Its product GET routes (`products.js`) are portal-staff-JWT-scoped (`requireRole`/`resolveTenantId`), not partner-API-key-scoped. **A new partner-scoped read endpoint** (e.g. `GET /api/partner/products`, gated by the same `partnerAuth.js` middleware `partnerProducts.js` already uses) needs building in `franky-forge` before JobMatix can pull anything.
   - JobMatix needs a small sync job (periodic or a manual "Sync Catalog" action) that calls that new endpoint and updates `stock.sellprice`/`description` for linked items.
...
**Not yet built** — this is a plan, not code. Next concrete step when picked up: build the partner read endpoint in `franky-forge` first (nothing on the JobMatix side is useful without it), then the `stock.franky_product_id` column + sync job here. Touches a live `franky-forge` production system (which itself shares a DB cluster with an unrelated live platform, per that project's own roadmap caution) — treat with the same care as any other production change, not as routine JobMatix dev work.
...
| Central catalog: reuse `franky-forge` (Phase 6.2) | Decided 2026-09-01 — reuse it, same product records as the Neto/Shopify catalog. Needs a new partner-scoped read endpoint built in `franky-forge` before any JobMatix-side sync code is useful. |
