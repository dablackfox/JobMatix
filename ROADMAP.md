...
**Pick up here next session** (2026-09-01): Brand/model reference data screens, Phase 6.1 (per-unit serial costing/COGS), and job reporting (Jobs/Parts/Staff/Timesheet) are all done. Phase 6.2 (central multi-store stock/pricing control via `franky-forge`) is scoped with a concrete plan and a decision made (same product records as the online catalog); the `franky-forge`-side read endpoint is built and tested but deliberately held back (not pushed to origin, no partner key issued) until ready to test end-to-end - do not push or otherwise touch `franky-forge` without an explicit go-ahead. Currently in progress: **job docket/quote printing** — the business owner's direction (2026-09-01): render real documents to PDF now so they can be reviewed, and explicitly defer (not stub) the physical hardware side (thermal/receipt printer, cash-drawer kick) given repeated real-world unreliability with services like Syncro's print integration (cash drawers routinely failing to open). Build real PDF rendering for the actual documents; leave one clear, explicit seam for swapping in real hardware output later, not a fake "Coming soon" placeholder. Other candidates after that: the two still-open plaintext-credential fixes from Phase 1 (`staff.password` hashing, `jobs.username`/`userpassword` encryption) - worth doing before any real deployment, not urgent for continued dev. See "Important, independently schedulable" and "Can ship separately" under Phase 3 below for the full remaining list.
